...
def normalize_rankings(tables: dict) -> pd.DataFrame:
    ...
    if not profiles.empty:
        ...
        if "player_strength" not in df.columns:
            sc = [c for c in ["avg_overall","avg_attack_overall","avg_defense_overall"] if c in df.columns]
            df["player_strength"] = df[sc].mean(axis=1) if sc else DEFAULT_PLAYER_STRENGTH
    ...
    if not form.empty:
        df = df.merge(form.rename(columns={"form_score":"form","win_pct":"win_pct"}), on="team", how="outer")
    if not rank.empty:
...
def team_stats(team: str, tables: dict, rankings: pd.DataFrame) -> dict:
    ...
    for src in [profile_row, form_row, rank_row, elo_row, ranking_row]:
        merged.update({k: v for k, v in src.items() if pd.notna(v) if not isinstance(v, float) or not np.isnan(v)})
...
def compute_power_index(rankings: pd.DataFrame, probs: pd.DataFrame) -> pd.DataFrame:
    ...
    df["win_pct"] = df["win_pct"].fillna(0.0) if "win_pct" in df.columns else 0.0
    df["fifa_rank"] = df["fifa_rank"].fillna(df["fifa_rank"].max() if "fifa_rank" in df.columns else DEFAULT_RANK)
    df["player_strength"] = df["player_strength"].fillna(DEFAULT_PLAYER_STRENGTH)
...
def build_profile(team: str, tables: dict, rankings: pd.DataFrame, probs: pd.DataFrame) -> dict:
    ...
    probability = row_for(probs, team).get("championship_probability", 0.0) if not probs.empty else 0.0
    atk = vfrom(profile, ["avg_attack_overall"], stats["player_strength"])
    dfn = vfrom(profile, ["avg_defense_overall"], stats["player_strength"])
    if atk <= 0: atk = min(max(stats["avg_goals_scored"] * 35.0, 35.0), 90.0)
    if dfn <= 0: dfn = min(max((3.5 - stats["avg_goals_conceded"]) * 22.0, 30.0), 90.0)
    return {
...
PLOTLY_LAYOUT = dict(
    ...
    title_font=dict(family="Bebas Neue", color="#ffffff", size=20, letterSpacing=2),
    legend=dict(bgcolor="rgba(0,0,0,0)", bordercolor="rgba(255,255,255,0.1)"),
    xaxis=dict(gridcolor="rgba(255,255,255,0.05)", linecolor="rgba(255,255,255,0.1)", tickfont=dict(color="#8899aa")),
    yaxis=dict(gridcolor="rgba(255,255,255,0.05)", linecolor="rgba(255,255,255,0.1)", tickfont=dict(color="#8899aa")),
...
def section_title(icon: str, text: str, highlight: str = ""):
    full = f"{icon} {text}" + (f" <span>{highlight}</span>" if highlight else "")
    st.markdown(f'<div class="section-title">{full}</div>', unsafe_allow_html=True)
...
def page_executive(tables, rankings):
    ...
    with k1: metric_card("Total Teams Analyzed", str(total_teams) if total_teams else "48", "National teams")
    with k2: metric_card("Matches in Dataset", "51,440", "Historical records since 1872")
    with k3: metric_card("Model Accuracy", "56.6%", "XGBoost — best 3-class", gold=True)
    with k4: metric_card("Simulation Runs", "10,000", "Monte Carlo tournament")

    # ── Top Spotlight ──
    if not probs.empty and not power_df.empty:
        section_title("🏆", "TOURNAMENT", "SPOTLIGHT")
        s1, s2, s3 = st.columns(3)
        top_champ = probs.sort_values("championship_probability", ascending=False).iloc[0]
        top_power = power_df.sort_values("power_index", ascending=False).iloc[0]
        top_form  = power_df.sort_values("form", ascending=False).iloc[0]
        with s1: metric_card("🥇 Top Championship Favorite",
                              f"{flag(top_champ['team'])} {top_champ['team']}",
                              f"{top_champ['championship_probability']:.2f}% win probability", gold=True)
        with s2: metric_card("⚡ Highest Power Index",
                              f"{flag(top_power['team'])} {top_power['team']}",
                              f"Power Index: {top_power['power_index']:.1f}")
        with s3: metric_card("🔥 Best Recent Form",
        ...
        fig.update_layout(yaxis_title="Win Probability (%)", **PLOTLY_LAYOUT)
        ...
        with c1:
            ...
            top20p = power_df.sort_values("power_index", ascending=False).head(20)
            ...
            fig2.update_layout(yaxis=dict(autorange="reversed"), xaxis_title="Power Index", **PLOTLY_LAYOUT)
        ...
        with c2:
            ...
            fig3 = px.scatter(
                ...
                hover_name="team",
                color_discrete_map={"UEFA":"#4488ff","CONMEBOL":"#44dd88","CONCACAF":"#ff8844","AFC":"#ff44aa","CAF":"#ffcc44"},
            )
            fig3.update_traces(opacity=0.85, marker=dict(line=dict(width=1, color="rgba(255,255,255,0.2)")))
            fig3.update_layout(xaxis_title="ELO Rating", yaxis_title="Championship %", **PLOTLY_LAYOUT)
        ...
        if not power_df.empty:
            pi_top = power_df.sort_values("power_index", ascending=False).iloc[0]
            insight(f"<b>{flag(pi_top['team'])} {pi_top['team']}</b> holds the #1 position in the composite World Cup Power Index, factoring ELO, form, squad strength, win rate, and FIFA rank.", "⚡")
            form_top = power_df.sort_values("form", ascending=False).iloc[0]
            insight(f"<b>{flag(form_top['team'])} {form_top['team']}</b> enters the tournament with the strongest recent form among all 2026 contenders.", "🔥")
            elo_top = power_df.sort_values("elo", ascending=False).iloc[0]
            insight(f"<b>{flag(elo_top['team'])} {elo_top['team']}</b> ranks #1 by ELO rating — a historically reliable predictor of international football dominance.", "🎯")
...
def page_match_predictor(tables, rankings):
    ...
    with c1:
        ...
        home_team = st.selectbox("🏠 Home Team", teams, index=default_h)
    with mid:
        st.markdown('<div class="vs-badge">VS</div>', unsafe_allow_html=True)
    with c2:
        default_a = teams.index("France") if "France" in teams else min(1, len(teams)-1)
        away_team = st.selectbox("✈️ Away Team", teams, index=default_a)
    ...
    pa = inject_power(build_profile(home_team, tables, rankings, probs_df), power_df)
    pb = inject_power(build_profile(away_team, tables, rankings, probs_df), power_df)
    ...
    if st.button("⚡ RUN PREDICTION", type="primary", use_container_width=True):
        try:
            X = construct_feature_row(home_team, away_team, int(nv), exp_features, tables, rankings)
            ...
            pred_label = {0: f"{away_team} Win", 1: "Draw", 2: f"{home_team} Win"}.get(pred, "Unknown")
            ...
            fig = go.Figure(go.Indicator(
                mode="gauge+number",
                value=home_pct,
                title=dict(text=f"<b>{home_team}</b> Win Probability", font=dict(color="#ffffff", family="Inter")),
                gauge=dict(
                    ...
                    threshold=dict(line=dict(color="#00E676", width=2), thickness=0.75, value=50)
                ),
                number=dict(suffix="%", font=dict(color="#FFD700", family="Bebas Neue", size=40))
            ))
            fig.update_layout(height=300, **{k: v for k, v in PLOTLY_LAYOUT.items() if k not in ("xaxis","yaxis")})
            st.plotly_chart(fig, use_container_width=True)
            ...
            stronger = home_team if (elo_gap > 0 and home_pct > away_pct) else away_team
    ...
    if not fi.empty:
        top = fi.sort_values("importance", ascending=False).head(15).sort_values("importance")
        fig = go.Figure(go.Bar(
        ...
        fig.update_layout(xaxis_title="Feature Importance", **PLOTLY_LAYOUT)
        st.plotly_chart(fig, use_container_width=True)
        ...
        st.info("Feature importance file not found at `dashboard/feature_importance.csv`.")
...
def page_team_intelligence(tables, rankings):
    ...
    mode = st.radio("Mode", ["Single Team Profile", "Head-to-Head Comparison"], horizontal=True)

    if mode == "Single Team Profile":
        selected = st.selectbox("Select Team", teams,
                                index=teams.index("Brazil") if "Brazil" in teams else 0)
        p = inject_power(build_profile(selected, tables, rankings, probs_df), power_df)
        ...
        with m1: metric_card("Championship %", f"{p['championship_probability']:.2f}%", gold=True)
        with m2: metric_card("Power Index", f"{p['power_index']:.1f}", "Composite score")
        with m3: metric_card("ELO Rating", f"{p['elo']:.0f}", f"Rank #{p['elo_rank']}")
        with m4: metric_card("Form Score", f"{p['form']:.2f}", f"Rank #{p['form_rank']}")
        with m5: metric_card("FIFA Rank", f"#{int(p['fifa_rank'])}", "Current ranking")
        ...
        for name, key, lo, hi in metrics:
            v = float(p.get(key, 0))
            if lo < hi: s = 100 * min(max((v - lo)/(hi - lo), 0), 1)
            else:       s = 100 * min(max((lo - v)/(lo - hi), 0), 1)
            scores.append(s); labels.append(name)
        scores.append(scores[0]); labels.append(labels[0])  # close polygon
        ...
        fig.update_layout(
            polar=dict(
                ...
                radialaxis=dict(visible=True, range=[0,100], tickfont=dict(color="#556677"), gridcolor="rgba(255,255,255,0.06)"),
                angularaxis=dict(tickfont=dict(color="#aabbcc", size=12), gridcolor="rgba(255,255,255,0.06)"),
            ),
            showlegend=False, height=450, **{k: v for k, v in PLOTLY_LAYOUT.items() if k not in ("xaxis","yaxis")}
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        with r1c1: metric_card("Win %",          f"{p['win_pct']:.1f}%",        "All-time record")
        with r1c2: metric_card("Goals Scored",   f"{p['avg_goals_scored']:.2f}", "Average per match")
        with r1c3: metric_card("Goals Conceded", f"{p['avg_goals_conceded']:.2f}", "Average per match")
        r2c1, r2c2, r2c3 = st.columns(3)
        with r2c1: metric_card("Player Strength", f"{p['player_strength']:.1f}",      "FIFA squad rating")
        with r2c2: metric_card("Team Strength",   f"{p['team_strength_score']:.1f}",  "Composite score")
        with r2c3: metric_card("FIFA Points",     f"{p['fifa_points']:.0f}",           "Ranking points")

    else:
        # ── Head-to-Head ──
        c1, c2 = st.columns(2)
        with c1:
            ta = st.selectbox("Team A", teams, index=teams.index("Argentina") if "Argentina" in teams else 0)
        with c2:
            tb = st.selectbox("Team B", teams, index=teams.index("France") if "France" in teams else min(1,len(teams)-1))
        if ta == tb:
            st.warning("Select two different teams.")
            return

        pa = inject_power(build_profile(ta, tables, rankings, probs_df), power_df)
        pb = inject_power(build_profile(tb, tables, rankings, probs_df), power_df)
        ...
        metrics = [
            ...
            ("Strength","player_strength",50,90), ("FIFA Rank","fifa_rank",200,1),
        ]
        traces = []
        for p_data, color in [(pa,"#FFD700"),(pb,"#00A8FF")]:
            ...
            for name, key, lo, hi in metrics:
                ...
                s = 100 * min(max((v-lo)/(hi-lo) if lo<hi else (lo-v)/(lo-hi), 0), 1)
                scores.append(s); labels.append(name)
            scores.append(scores[0]); labels.append(labels[0])
            traces.append(go.Scatterpolar(
                r=scores, theta=labels, fill="toself",
                fillcolor=color.replace("#","rgba(").replace("FFD700","255,215,0,0.1").replace("00A8FF","0,168,255,0.1") + ")" if color in ["#FFD700","#00A8FF"] else "rgba(0,0,0,0.1)",
        ...
        fig.update_layout(
            ...
            height=450, **{k: v for k, v in PLOTLY_LAYOUT.items() if k not in ("xaxis","yaxis")}
        )
        st.plotly_chart(fig, use_container_width=True)
        ...
        metrics_table = [
            ("ELO Rating","elo"), ("FIFA Rank","fifa_rank"), ("Form Score","form"),
            ("Win %","win_pct"), ("Avg Goals Scored","avg_goals_scored"),
            ("Avg Goals Conceded","avg_goals_conceded"), ("Player Strength","player_strength"),
        ...
        for label, key in metrics_table:
            ...
            better_a = (va > vb and key != "fifa_rank") or (va < vb and key == "fifa_rank")
            ca, cb = ("var(--gold)" if better_a else "var(--white)"), ("var(--gold)" if not better_a else "var(--white)")
            col1, col2, col3 = st.columns([2,1,1])
            with col1: st.markdown(f"<div style='color:var(--grey);font-size:0.85rem;padding:0.4rem 0'>{label}</div>", unsafe_allow_html=True)
            with col2: st.markdown(f"<div style='color:{ca};font-size:0.95rem;font-weight:700;padding:0.4rem 0'>{va:.2f}</div>", unsafe_allow_html=True)
            with col3: st.markdown(f"<div style='color:{cb};font-size:0.95rem;font-weight:700;padding:0.4rem 0'>{vb:.2f}</div>", unsafe_allow_html=True)
...
def page_simulation(tables, rankings):
    ...
    probs = read_csv(DASHBOARD_DIR / "championship_probabilities.csv")
    if probs.empty:
        st.warning("No championship probabilities found. Run `simulation/monte_carlo.py` first.")
        return
    power_df = compute_power_index(rankings, probs)
    ...
    run_sim = st.selectbox("Simulation Run Count (display reference)", ["1,000","5,000","10,000 (current)","50,000"], index=2)
    ...
    with col_pie:
        ...
        pie = go.Figure(go.Pie(
            labels=[f"{flag(r['team'])} {r['team']}" for _,r in top20.iterrows()],
        ...
        pie.update_layout(showlegend=False, height=400,
    ...
    with col_table:
        ...
        display["championship_probability"] = display["championship_probability"].map(lambda v: f"{v:.2f}%")
    ...
    if len(probs) >= 3:
        top3names = ", ".join([f"{flag(r['team'])} {r['team']}" for _,r in probs.head(3).iterrows()])
    ...
    if not power_df.empty:
        ...
        if not eu_teams.empty and not sa_teams.empty:
            eu_pct = probs[probs["team"].isin(eu_teams["team"])]["championship_probability"].sum()
            sa_pct = probs[probs["team"].isin(sa_teams["team"])]["championship_probability"].sum()
            insight(f"UEFA teams hold <b>{eu_pct:.1f}%</b> combined probability; CONMEBOL teams hold <b>{sa_pct:.1f}%</b> — a classic Europe vs South America WC rivalry.", "🌍")
    if not power_df.empty:
        dark_horse = probs[probs["championship_probability"] < 5].sort_values("championship_probability", ascending=False)
    ...
    if not power_df.empty:
        ...
        fig2.update_layout(xaxis_title="Power Index", yaxis_title="Championship Probability (%)", **PLOTLY_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)
...
def page_model(tables, rankings):
    ...
    with m1: metric_card("XGBoost Accuracy", "56.6%", "Best model", gold=True)
    with m2: metric_card("F1 Score (macro)", "0.462", "XGBoost")
    with m3: metric_card("CV Accuracy", "53.0%", "Cross-validated")
    with m4: metric_card("Baseline", "~43%", "Random 3-class")
    ...
    for i, col in enumerate(["Accuracy","F1 Score (×100)","CV Accuracy"]):
        y = model_data["Accuracy"] if col=="Accuracy" else \
            model_data["F1 Score"]*100 if "F1" in col else model_data["CV Accuracy"]
    ...
    fig.update_layout(barmode="group", yaxis_title="Score (%)", legend=dict(font=dict(color="#ffffff")), **PLOTLY_LAYOUT)
    st.plotly_chart(fig, use_container_width=True)
    ...
    if not fi.empty:
        top = fi.sort_values("importance", ascending=False).head(20).sort_values("importance")
        fig2 = go.Figure(go.Bar(
            x=top["importance"], y=top["feature"], orientation="h",
            marker=dict(color=top["importance"],
                        colorscale=[[0,"#002255"],[0.5,"#0055aa"],[1,"#FFD700"]], showscale=True,
                        colorbar=dict(title="Importance",tickfont=dict(color="#8899aa"),titlefont=dict(color="#8899aa"))),
            hovertemplate="<b>%{y}</b><br>Importance: %{x:.4f}<extra></extra>"
        ))
        fig2.update_layout(xaxis_title="Feature Importance", height=600, **PLOTLY_LAYOUT)
        st.plotly_chart(fig2, use_container_width=True)
    else:
...
def page_about():
    ...
    for group, techs in tech_groups.items():
        st.markdown(f"<div style='color:var(--gold);font-size:0.8rem;font-weight:700;letter-spacing:1px;margin:1rem 0 0.5rem'>{group.upper()}</div>", unsafe_allow_html=True)
    ...
    with d1: metric_card("Total Matches", "51,440", "Since 1872", gold=True)
    with d2: metric_card("WC Matches", "836", "1930–2022")
    with d3: metric_card("FIFA Rankings", "6,675", "Historical rows")
    with d4: metric_card("Player Records", "~3,000", "Squad aggregates")
    with d5: metric_card("Country Mappings", "36", "Former names")
...
def page_team_rankings(tables, rankings):
    ...
    query = st.text_input("🔍 Search team", placeholder="Type a team or confederation...")
    view = power_df.copy() if not power_df.empty else rankings.copy()
    if query:
        view = view[view.apply(lambda r: query.lower() in str(r).lower(), axis=1)]
    ...
    if not rankings.empty:
        ...
        if "elo" in rankings.columns:
            ...
            with m1: metric_card("Top ELO", f"{flag(top_e['team'])} {top_e['team']}", f"{top_e['elo']:.0f}")
        if "fifa_rank" in rankings.columns:
            ranked = rankings[rankings["fifa_rank"].notna()].sort_values("fifa_rank")
            if not ranked.empty:
                top_r = ranked.iloc[0]
                with m2: metric_card("Best FIFA Rank", f"{flag(top_r['team'])} {top_r['team']}", f"Rank #{int(top_r['fifa_rank'])}")
        if "form" in rankings.columns:
            top_f = rankings.sort_values("form", ascending=False).iloc[0]
            with m3: metric_card("Best Form", f"{flag(top_f['team'])} {top_f['team']}", f"{top_f['form']:.2f}", gold=True)

    # Table
    if not view.empty:
        show_cols = [c for c in ["team","confederation","elo","fifa_rank","form","player_strength","championship_probability","power_index"] if c in view.columns]
    ...
    with chart_cols[0]:
        if "elo" in rankings.columns:
            ...
            fig.update_layout(title_text="TOP TEAMS BY ELO", **PLOTLY_LAYOUT)
            st.plotly_chart(fig, use_container_width=True)
    with chart_cols[1]:
        if "fifa_rank" in rankings.columns:
            top = rankings[rankings["fifa_rank"].notna()].sort_values("fifa_rank").head(15)
            ...
            fig.update_layout(title_text="BEST FIFA RANKINGS", yaxis=dict(autorange="reversed"), **PLOTLY_LAYOUT)
            st.plotly_chart(fig, use_container_width=True)
    with chart_cols[2]:
        if "form" in rankings.columns:
            ...
            fig.update_layout(title_text="BEST RECENT FORM", **PLOTLY_LAYOUT)
            st.plotly_chart(fig, use_container_width=True)
...
def sidebar_nav():
    with st.sidebar:
        st.markdown('<div class="sidebar-logo">⚽ FIFA WC 2026</div>', unsafe_allow_html=True)
        ...
        status_items = [
            ...
            ("Championship Probs", DASHBOARD_DIR / "championship_probabilities.csv"),
